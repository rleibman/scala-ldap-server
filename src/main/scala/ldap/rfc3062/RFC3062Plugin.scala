...
object RFC3062Plugin extends Plugin {
  ...
  override def supportedExtensions: Seq[ldap.SupportedExtension] = Seq(SupportedExtension(oid, "Password ModifY Request"))
      ...
      case 23 => //Extended
        applicationAsn1.value.toSeq match {
          case Seq(Asn1String(oid.value), details: Asn1String) ⇒
            ...
            extended match {
              case List(Asn1String(userIdentity), Asn1String(oldPassword), Asn1String(requestedPassword)) =>
  ...
  override def operate(msg: LdapMessage, preResults: Seq[LdapMessage], dao: DAO): Future[Seq[LdapMessage]] = Future.successful {
    msg.protocolOp match {
      case ChangePasswordRequest(userIdentity, oldPassword, requestedPassword) =>
        ...
        val valid = passwordPolicy.validatePassword(oldPassword, requestedPassword)
        val response = if (valid.isEmpty) {
          if (userIdentity.isEmpty()) {
            val result = LdapResult(LDAPResultType.noSuchObject, userIdentity, s"Could not change password: ${valid.mkString(".")}, user is empty")
            ...
            if (requestedPassword.isEmpty && passwordPolicy.generatePasswordIfEmpty) {
              ...
              //TODO save password
              val result = LdapResult(LDAPResultType.success, userIdentity, "Password changed successfully")
              LdapMessage(msg.messageId, ChangePasswordResponse(result, Some(generatedPassword)))
            } else if (requestedPassword.isEmpty) {
              val result = LdapResult(LDAPResultType.unwillingToPerform, userIdentity, s"Could not change password: You may not set the password to blank")
              LdapMessage(msg.messageId, ChangePasswordResponse(result, None))
            } else {
              //TODO save password
              val result = LdapResult(LDAPResultType.success, userIdentity, "Password changed successfully")
          ...
          val result = LdapResult(LDAPResultType.unwillingToPerform, userIdentity, s"Could not change password: ${valid.mkString(".")}")
          LdapMessage(msg.messageId, ChangePasswordResponse(result, None))
        }
        preResults :+ response
