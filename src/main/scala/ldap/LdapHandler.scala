...
case class LdapHandler(client: Option[InetSocketAddress]) extends Actor with Config {
  ...
  val sessionManager = context.actorSelection(s"/user/${LdapSessionManager.name}")
  ...
  def operate(msg: LdapMessage): Future[Seq[LdapMessage]] = {
    ...
    for {
      session: Option[LdapSession] <- client.fold(Future.successful(None: Option[LdapSession]))(
        client => (sessionManager ? LdapSessionManager.GetSession(client)).mapTo[Option[LdapSession]]
      )
      returnMessages <- {
        ...
        plugins.foldLeft(returnMessagesFut)((z, plugin) => z.flatMap(preResults => plugin.operate(msg, preResults, dao)))
      }
    } yield {
      session.fold(returnMessages) { session =>
        if (returnMessages.isEmpty) {
          ...
          client.map(client => sessionManager ! LdapSessionManager.EndSession(client))
          Seq(LdapMessage(msg.messageId, NoticeOfDisconnection(LdapResult(LDAPResultType.protocolError, "", s"${msg.protocolOp} not handled"))))
        } else {
