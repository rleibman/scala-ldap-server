...
case class LdapSaslAuthentication(mechanism: String, credentials: Option[String] = None)
...
case class EqualityMatchFilter(attributeDescription: String, attributeValue: String)
...
case class GreaterOrEqualFilter(attributeDescription: String, attributeValue: String)
    extends Filter(FilterType.greaterOrEqual)
case class LessOrEqualFilter(attributeDescription: String, attributeValue: String)
    extends Filter(FilterType.lessOrEqual)
case class PresentFilter(str: String) extends Filter(FilterType.present)
case class AproxMatchFilter(attributeDescription: String, attributeValue: String)
...
case class SearchResultEntry(uuid: UUID, dn: String, attributes: Map[String, Seq[String]])
...
case class BindResponse(ldapResult: LdapResult, serverSaslCreds: Option[String] = None)
    extends Response
...
case class CompareRequest(dn: String, attributeDescription: String, attributeValue: String)
    extends Request
...
case object RootNode extends ServerStructuralNode {
  val supportedControls = List(
    SupportedControl(LDAPOID("2.16.840.1.113730.3.4.18"),
  ...
  override val userAttributes = Map(
    ...
    "supportedControl" -> (supportedControls ++ plugins.flatMap(_.supportedControls))
    ...
    "supportedExtension" -> (supportedExtensions ++ plugins.flatMap(_.supportedExtensions))
      .map(_.oid.value), //TODO this is a dynamic value, should it be saved? calculated? calculated AND saved? saved when a new plugin is instnalled?
    "supportedFeature" -> (supportedFeatures ++ plugins.flatMap(_.supportedFeatures))
      .map(_.oid.value), //TODO this is a dynamic value, should it be saved? calculated? calculated AND saved? saved when a new plugin is instnalled?
