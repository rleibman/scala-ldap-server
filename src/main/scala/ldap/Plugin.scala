...
trait Plugin {
  ...
  def decodeApplication(
      applicationAsn1: Asn1Application): Option[MessageProtocolOp] = None
  def decodeControl(controlAsn1: Asn1Object): Option[Control] = None
  def operate(msg: LdapMessage,
  ...
  def readSchemaFile(file: File) = {
    ...
    resources.flatten.foldLeft(
      (Seq.empty[LdapSyntax],
       ...
       Seq.empty[ObjectClass])) { (previous, resource) =>
      resource match {
        case resource: LdapSyntax =>
          (previous._1 :+ resource,
           previous._2,
           previous._3,
           previous._4,
           previous._5)
