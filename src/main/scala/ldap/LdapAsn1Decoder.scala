...
object LdapAsn1Decoder extends Config {
  private def encode(filter: Filter): Asn1ContextSpecific = filter match {
    ...
    case OrFilter(filters) =>
      throw new Error(s"Unhandled Filter Type ${filter}")
    //TODO
    case AndFilter(filters) =>
      throw new Error(s"Unhandled Filter Type ${filter}")
    //TODO
    case NotFilter(filter) =>
      throw new Error(s"Unhandled Filter Type ${filter}")
    //TODO
    ...
    val operation: MessageProtocolOp = applicationAsn1.tag match {
      ...
      case 3 ⇒ {
        val attributes = if (values.length > 7) {
          values(7)
            .asInstanceOf[Asn1Sequence]
            .value
            .map(_.asInstanceOf[Asn1String].value)
        } else {
          Seq.empty
        }
        SearchRequest(
          ...
          values(1)
            ...
            .map { value1 =>
              (value1
                 .asInstanceOf[Asn1Sequence]
                 .value(0)
                 .asInstanceOf[Asn1String]
                 .value,
               value1
                 .asInstanceOf[Asn1Sequence]
                 .value(1)
                 .asInstanceOf[Asn1Set]
                 .value
                 .map(_.asInstanceOf[Asn1String].value)
                 .toSeq)
            }
      ...
      case 5 =>
        val referral = if (values.length > 3) {
          (values(3)
            .asInstanceOf[Seq[Asn1String]]
            .map(aString => new URI(aString.value)))
            .toList
        } else {
          List.empty
        }
        val result = LdapResult(
          LDAPResultType(values(0).asInstanceOf[Asn1Enumerated].value),
          values(1).asInstanceOf[Asn1String].value,
          values(2).asInstanceOf[Asn1String].value,
          referral
        )
