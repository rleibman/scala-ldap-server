...
class RFC3062Spec extends TestKit(ActorSystem("MySpec")) with FlatSpecLike with ImplicitSender with BeforeAndAfterAll {
  ...
  "sending a simple password request change when user is empty" should "error out" in {
    ...
    val response = responseMsg.head.protocolOp.asInstanceOf[ChangePasswordResponse]
  ...
  "sending a simple password request change when user does not exist" should "error out" in {
    val request = ChangePasswordRequest(userIdentity = "non-existing", oldPassword = "", newPassword = "")
    handler ! LdapMessage(
      123,
      request
    )
  ...
  "sending a simple password request change when password doesn't match" should "error out" in {
    val request = ChangePasswordRequest(userIdentity = existingUserDn, oldPassword = "", newPassword = "")
    handler ! LdapMessage(
      123,
      request
    )
  ...
  "sending a simple password request change when new password is ugly" should "error out" in {
    val request = ChangePasswordRequest(userIdentity = existingUserDn, oldPassword = "", newPassword = "aoeu")
    handler ! LdapMessage(
      123,
      request
    )
